...
class Shiplisting_Api {
        ...
        if ( sizeof( $boat_data[ 'pictures' ] ) > 0 ) {
            ...
            <div class="shiplisting-images-slider-wrapper rounded-corners">
                ...
                foreach($boat_data[ 'pictures' ] as $key => $picture) {
                        ...
                        $boat_data["imageSlider"]["html"] .=  '<li class="shiplisting-images-slider-item'.$current.'"><img src="'.str_replace('huge_', 'list_', $picture['val']).'" /></li>';
                }
            $boat_data["imageSlider"]["html"] .= '
                </ul>
            </div>
    ...
    public function get_boat_images($bid, $size="huge") {
        $tmpBoatImages = json_decode($this->send_request('boat-data', 'pictures', '&bid='.$bid.'&pic_size='.$size.''));
    ...
    public function ajax_get_all_filters($fieldtype=false) {
        $tmpBoat = json_decode( $this->send_request( 'boat-data', 'all-filters', ($fieldtype) ? '&fieldtype=search' : '' ), true );
        if ( !$tmpBoat ) {
            return;
        }
        ...
        foreach ($tmpBoat['selects'] as $key => $value) {
            ...
            if (is_array($value)) {
                ...
                if (is_numeric($value[0]['name']) && !in_array($key, $numeric_exceptions)) {
                    ...
                    $tmp_select_store = '';
                    $unit_finder = (in_array($key, $length_fields)) ? ' ('.$tmpTranslation['words']['m'].')' : '' ;
        ...
        if ($result) {
            if ($result[0]->{'id'}) {
                ...
                Shiplisting_Shortcodes::$api->currentRoute = [
                    'id' => $result[0]->{'id'},
                    'path' => $current_uri,
                    'adv_filter' => ($result[0]->{'adv_filter'}) ? implode(',', json_decode($result[0]->{'adv_filter'}, true)) : '',
        ...
        if ($this->get_current_route('', $routes_id)) {
            ...
            if (!empty($tmp_title)) {
                ...
                if (sizeof($fields) > 0) {
                    ...
                    // send request to api
                    $data = json_decode($this->send_request('info-special', 'get-placeholders', '&bid='.$bid.'&fields='.implode(',', $fields)), true);
                    $data = $data['fields'];
...
class Shiplisting_Router {
    ...
    public static function generate_routes(WP_Router $router) {
        ...
        foreach ($result as $route) {
            ...
            $router->add_route($route->{'name'}, array(
                ...
                'page_arguments' => (strlen($route->{'arguments'}) == 0) ? null : json_decode($route->{'arguments'}, true),
    ...
    public static function title_callback($boat_id) {
        ...
        return Shiplisting_Shortcodes::$api->resolve_title_placeholders(Shiplisting_Shortcodes::$api->currentRoute['id'], $boat_id);
    }
    ...
    private static function replace_placeholder_boat_details($boat_id, $content, $source) {
        ...
        if (!empty($source)) {
            if ((int)$source == Shiplisting_SourceType::YACHTALL) {
                Shiplisting_Shortcodes::$api->update_uri('https://api.yachtall.com/');
            } else if ((int)$source == Shiplisting_SourceType::HAPPYCHARTER) {
                Shiplisting_Shortcodes::$api->update_uri('https://api.happycharter.com/');
            }
        }
            ...
            if ($placeholder == "season_prices") {
                ...
                foreach ( $boat_data[ 'charter_data' ][ 'prices' ][ 'val' ] as $key => $type ) {
                    if (is_array($type[ 'season_prices' ])) {
                        ...
                        $boxTmpl .= str_replace('XXX_curr', $boat_data['charter_data']['prices']['attr']['currency_sign'], self::$translation['boat']['charter_data'][$build_key]);
                        $boxTmpl .= '</div>';
    ...
    private static function replace_placeholder_boats($boatTemplate, $content, $filterId, $hitsByPage, $source) {
        ...
        if ($boat_data) {
            foreach ( $boat_data as $boatObj ) {
                ...
                if ( !empty( $template ) ) {
                    $template = preg_replace_callback( "/\{(.*?)\}/i", function ( $result ) use ( $boatObj, $boatTemplate, $boatOrg, $source ) {
                        $placeholder = $result[ 1 ];
                        $isTranslation = false;
        ...
        echo preg_replace_callback("/\{(.*?)\}/i", function ($result) use ($boat_data, $boatTemplate, $pagination, $source, $plugin_data) {
            $placeholder = $result[1];
            $isTranslation = false;
            ...
            if ($placeholder == "filtering" && !empty(Shiplisting_Shortcodes::$api->currentRoute['adv_filter'])) {
                if ($source != "") {
                    ...
                    $template = preg_replace_callback("/\{(.*?)\}/i", function ($filter_result) use ($boat_data, $boatTemplate, $pagination, $source) {
